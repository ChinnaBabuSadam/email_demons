...
struct email_struct* createEmailList(int val, char *_myEmailContent,
		int isLocal) {
	//printf("\n creating list with headnode as [%d]\n", val);
	struct email_struct *ptr = (struct email_struct*) malloc(
			sizeof(struct email_struct));
...
struct email_struct* searchEmail(int val, struct email_struct **prev,
		int isLocal) {
	...
	while (ptr != NULL) {
		...
		if (option == 1) {
			if (strcmp(status, query) == 0) {
				foundEmail += 1;
				printf(
						"[%d]:%s From: %s - Email: %s - Subject: %s - Time: %s\n",
...
void DisplayEmail() {
	if (foundEmail > 0) {
		...
		int mailNumber;
		printf(
				"Please type the number of email that you want to read (-1 to exit): ");
		scanf("%d", &mailNumber);
		getchar();
		if (mailNumber != -1) {
...
void DoReadEmail() {
	...
	while (isInReadingEmailProcess) {
		...
		if (readingoption == -1) {
			...
			printf(
					"Type 1 to filter by reading status, 2 by sender name, 3 by subject (-1 to exit): ");
			scanf("%d", &filteroption);
			getchar();
			if (filteroption == 1) {
				printf(
						"Please type ~Un-Read for unread email, ~Read for already read email (NULL to exit): ");
				scanf("%[^\n]s", filerString);
				getchar();
...
void MainScreen() {
	...
	do {
		if (isLogin == 1) {
			printf(
					"Type 1 for compose email, 2 for checking email (-1 to quit): ");
...
void doClientRegistration() {
	...
	sprintf(client_message, "%d:%s:%s:%s:%s", 2, currentUserName, pwd,
...
void doClientLogin() {
	...
	while (!isLogin) {
		...
		if (strcmp(returnMessage, "NONE") != 0) {
			...
			sscanf(returnMessage, "%d:%[^:]:%[^:]", &isLogin, currentUserFName,
					currentUserLName);
			if (isLogin) {
