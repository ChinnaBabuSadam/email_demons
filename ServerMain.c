...
void serverDoRegistration(char *message, int sk) {
	...
	sscanf(message, "%[^':']:%[^':']:%[^':']:%s", userName, pwd, firstName,
			lastName);
		...
		while (fgets(readLine, MAX_LINE, fp) != NULL) {
			if (!isInAttachement) {
				...
				if (strstr(readLine, "#END") == NULL) {
					...
					sprintf(UserDisplayEmail, "%s%s%s%s%s%s%s", status, from,
							senderEMail, sendTime, subject, emailContent,
							attachment);
					if (send(sk, UserDisplayEmail, MAX_CONTENT, 0) < 0) {
						printf(
								"SERVER: can't send reply authentication to client\n");
						close(sk);
						exit(EXIT_FAILURE);
					}
	...
	for (;;) {
		...
		if ((childpid = fork()) < 0) {
			...
			for (;;) {
				...
				int n = sscanf(child_Message, "%d:%[^'\0']", &option,
						child_Main_Message);
